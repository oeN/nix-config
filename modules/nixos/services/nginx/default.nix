...
in {
  ...
  options.my.services.nginx = with lib; {
    ...
    acme = {
      ...
      wildcardDomain = mkOption {
        ...
        example = "example.com";
        description = ''
          The wildcard domain to use for the ACME certificate.
        '';
      };
    };
  ...
  config = lib.mkIf cfg.enable {
    assertions =
      []
      ++ (lib.flip builtins.map cfg.virtualHosts
        ({subdomain, ...} @ args: let
      ...
      ++ (let
        ports =
          lib.my.mapFilter (v: v != null) ({port, ...}: port)
          cfg.virtualHosts;
        ...
        mkVHost = {subdomain, ...} @ args:
          lib.nameValuePair "${subdomain}.${domain}" (lib.my.recursiveMerge [
            ...
            (lib.optionalAttrs (args.socket != null) {
              locations."/".proxyPass = "http://unix:${args.socket}";
            })
        ...
        configuration = {
          ...
          providers = {
            ...
            in {
              ...
              # mfa = applyUsers (_: v: [{
              #   provider = "totp";
              #   attributes = { secret = { _secret = v.totpSecretFile; }; };
              # }]);
    ...
    security.acme = {
      acceptTerms = true;
      defaults.email = "diomede.tripicchio+acme@gmail.com";
      certs = {
        "${domain}" = {
          domain = "*.${cfg.acme.wildcardDomain}";
          dnsProvider = "route53";
